import express, { NextFunction, Request, Response } from "express";
import Category from "../model/category";

import categoryService from '../service/category';
import ResponseCode from "../utils/code";
import { checkParamsIsNull, filterParams, getCurrentDateTime, handleErrorNext } from "../utils/index";

const router = express.Router();

router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const categorys = await categoryService.find()

    res.json({
      code: ResponseCode.SUCCESS,
      data: categorys || null
    })
  } catch(err) {
    handleErrorNext(err, next);
  }
});

router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const category = await categoryService.findCategoryById(Number(req.params.id))
  
    res.json({
      code: ResponseCode.SUCCESS,
      data: category || null
    })
  } catch(err) {
    handleErrorNext(err, next)
  }
});

router.post('/add', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const _category = filterParams<Category>(req.body, Category);
    await checkParamsIsNull(_category, ['title'])
  
    const category = await categoryService.addCategory(_category).catch(err => {
      throw { msg: err, code: ResponseCode.SERVICE_ERROR }
    });
    
    res.json({
      code: 200,
      data: category
    })
  } catch(err: any) {
    handleErrorNext(err, next);
  }
});

router.put('/update', async(req: Request, res: Response, next: NextFunction) => {
  try {
    const _category = filterParams<Category>(req.body, Category);
    await checkParamsIsNull(_category, ['id', 'title', 'parentId'])
  
    const category = await categoryService.updateCategory(_category).catch(err => {
      throw { msg: err, code: ResponseCode.INVALID_PARAMS }
    });
    
    res.json({
      code: 200,
      data: category
    })
  } catch(err) {
    handleErrorNext(err, next);
  }
})

router.delete('/remove/:id', async(req: Request, res: Response, next: NextFunction) => {
  try {
    const id = Number(req.params.id);

    const affected = await categoryService.removeCategory(id).catch(err => {
      throw { msg: err, code: ResponseCode.SERVICE_ERROR }
    })
  
    if (!affected || affected === 0) {
      throw { msg: '该标签已不存在', code: ResponseCode.DATA_IS_EMPTY }
    }
  
    res.json({
      code: ResponseCode.SUCCESS,
      data: req.params.id
    })
  } catch(err) {
    handleErrorNext(err, next);
  }
})
export default router;
